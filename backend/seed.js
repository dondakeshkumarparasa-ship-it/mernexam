...
const seedDB = async () => {
  try {
    ...
    const created = await Question.insertMany(seedQuestions);
    console.log(`Successfully seeded ${created.length} competitive exam questions into MongoDB!`);
    
    // Seed default administrative user if not exists
    const adminEmail = 'dondakeshkumarparasa@gmail.com';
    // Clear any conflicting admin user to guarantee fresh seed
    ...
    await admin.save();
    console.log('Successfully seeded default administrative user profile:');
    console.log('  Email:    dondakeshkumarparasa@gmail.com');
