...
window.ExamPulse = (function() {
  ...
  async function addNotificationAsync(notifData) {
    if (window.location.protocol.startsWith('http') || liveApiBaseUrl) {
      try {
        ...
        console.warn("Network fetch failed or non-JSON served. Falling back to local storage direct inject.", e);
        return saveNotificationLocally(notifData);
      }
    } else {
      return saveNotificationLocally(notifData);
    }
  ...
  function saveNotificationLocally(notifData) {
